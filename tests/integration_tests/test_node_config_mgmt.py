...
def context():
    ...
    node = Node(
        id=uuid4(), name="default_node", node_type="default_type", attributes={}
    )
...
@given(
    'the system has existing node types: "Human", "AI Agent", "Internal API Endpoint", "External API Endpoint", "Group"'
)
...
@when(
    parsers.parse(
        'an administrator configures a new node of type "{node_type}" with attributes'
    )
)
...
def verify_node_added(context):
    ...
    assert (
        context["node"].id == "node_1"
    )  # Placeholder, actual verification logic needed
